...
#Example Cronjob: '00  2   *   *   *   /usr/local/sbin/automysqlbackup.sh'
...
MAILADDR=
# System command to dump mysql data
MYSQLDUMPCOMMAND="mysqldump"

while getopts 'u:p:h:n:d:m:c:' OPTION
do
  case $OPTION in
  ...
  m)  MAILADDR=$OPTARG
                ;;
  c)  MYSQLDUMPCOMMAND=$OPTARG
                ;;
  ?)    printf "Usage: %s: [-u username] [-p password] [-h db_host] [-n db_names] [-d backup_dir] [-m email_address] [-c mysql_dump_command (mysqldump)] args\n" $(basename $0) >&2
                exit 2
                ;;
  esac
done
...
MDBNAMES="mysql $DBNAMES" #includes mysql config database in monthly backup
...
dbdump () {
$MYSQLDUMPCOMMAND --user=$USERNAME --password=$PASSWORD --host=$DBHOST $OPT $1 > $2
return 0
}
